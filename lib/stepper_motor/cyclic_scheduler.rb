...
class StepperMotor::CyclicScheduler < StepperMotor::ForwardScheduler
  class RunSchedulingCycleJob < ActiveJob::Base
    def perform
      StepperMotor.scheduler&.run_scheduling_cycle
    end
  end
  ...
  def run_scheduling_cycle
    ...
    # which haven't been scheduled or weren't scheduled on time. We don't want to only schedule journeys which are "past due",
    # because this would make the timing of the steps very lax.
